...
# p2: 64MB squashfs root - future-proof for kernel growth - kernel mounts as root=/dev/mmcblk0p2
# p3: 32MB boot - U-Boot loads kernel from mmc 0:3 (REQUIRED)
# p4: 2GB ext4 rootfs_data - expandable - LAST PARTITION = EXPANDABLE!

p1_start=2048
p1_sectors=2048  # 1MB placeholder
p2_start=$((p1_start + p1_sectors))
p2_sectors=131072  # 64MB squashfs (plenty of headroom)
p3_start=$((p2_start + p2_sectors))
p3_sectors=65536  # 32MB boot
p4_start=$((p3_start + p3_sectors))
p4_sectors=4194304  # 2GB rootfs_data (expandable!)
